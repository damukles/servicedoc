...
type alias Customizations data msg =
    ...
    , thead : List ( String, Status, Maybe String, Attribute msg ) -> HtmlDetails msg
...
simpleThead : List ( String, Status, Maybe String, Attribute msg ) -> HtmlDetails msg
simpleThead headers =
    HtmlDetails [] (List.map simpleTheadHelp headers)


simpleTheadHelp : ( String, Status, Maybe String, Attribute msg ) -> Html msg
simpleTheadHelp ( name, status, colClass, onClick ) =
    let
        content =
            case status of
                Unsortable ->
    ...
    in
        Html.th [ onClick, colClassAttr colClass ] content


colClassAttr : Maybe String -> Attribute msg
colClassAttr =
    Attr.class << Maybe.withDefault ""
...
type alias ColumnData data msg =
    ...
    , sorter : Sorter data
    , colClass : Maybe String
    }


{-| -}
stringColumn : String -> Maybe String -> (data -> String) -> Column data msg
stringColumn name colClass toStr =
    Column
        { name = name
...
{-| -}
intColumn : String -> Maybe String -> (data -> Int) -> Column data msg
intColumn name colClass toInt =
    Column
        { name = name
...
{-| -}
floatColumn : String -> Maybe String -> (data -> Float) -> Column data msg
floatColumn name colClass toFloat =
    Column
        { name = name
...
customColumn :
    ...
    -> Column data msg
customColumn { name, viewData, sorter, colClass } =
    Column <|
        ColumnData name (textDetails << viewData) sorter colClass
...
toHeaderInfo : State -> (State -> msg) -> ColumnData data msg -> ( String, Status, Maybe String, Attribute msg )
toHeaderInfo (State sortName isReversed) toMsg { name, sorter, colClass } =
    case sorter of
        None ->
            ( name, Unsortable, colClass, onClick sortName isReversed toMsg )

        Increasing _ ->
            ( name, Sortable (name == sortName), colClass, onClick name False toMsg )
        ...
        IncOrDec _ ->
            if name == sortName then
                ( name, Reversible (Just isReversed), colClass, onClick name (not isReversed) toMsg )
            else
                ( name, Reversible Nothing, colClass, onClick name False toMsg )
...
viewCell data { viewData, colClass } =
    let
        details =
            viewData data

        attributes =
            colClassAttr colClass :: details.attributes
    in
        Html.td attributes details.children
